...
export default function VisionInterface({ user }: VisionInterfaceProps) {
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/${params.siteId}/vision/`, {
        method: "POST",
      ...
      setMessages((prevMessages) => {
        const updatedMessages = prevMessages.map((msg): Message => {
          if (
            msg === prevMessages[prevMessages.length - 1] &&
            msg.sender === "user"
          ) {
            return { ...msg, image: msg.tempImage, tempImage: undefined };
          }
          return msg;
        });
